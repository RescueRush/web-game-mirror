...
//@formatter:off
@DB_View(name = "execution_monitor_summary_per_key_per_day", tables = {
		@ViewTable(typeName = ExecutionMonitorTable.class, columns = {
				@ViewColumn(name = "key"),
				@ViewColumn(func = "DATE(time_start)", asName = "date"),
...
orderBy = { @OrderBy(column = "date"), @OrderBy(column = "key") })
//@formatter:on.
@Service
public class ExecutionMonitor_SummaryPerKeyPerDay_View extends R2DBView<ExecutionMonitorData> {

	@Autowired
	private ExecutionMonitorTable perfs;

	public ExecutionMonitor_SummaryPerKeyPerDay_View(DataBase dbTest) {
		super(dbTest);
	}
