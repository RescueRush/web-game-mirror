...
//@formatter:off
@DB_View(name = "game_stats_filtered_leaderboard", tables = {
		@ViewTable(typeName = GameProgress_Stats_View.class, columns = {
				@ViewColumn(name = "*"),
...
}, condition = "NOT EXISTS (SELECT 1 FROM `user_sanction` WHERE `user_sanction`.`user_id` = `u`.`id` AND `user_sanction`.`reason_id` IN (SELECT `id` FROM `user_sanction_reason` WHERE `key` = 'ban' OR `key` = 'hidden'))")
//@formatter:on
@Service
public class GameStats_FilteredLeaderboard_View extends R2DBView<LeaderboardData> implements LeaderboardView {

	public static final int ENTRIES_PER_PAGE = 15;

	@Autowired
	private GameProgress_Stats_View quizGameStats;
	@Autowired
	...
	public GameStats_FilteredLeaderboard_View(DataBase dbTest) {
		super(dbTest);
	}
